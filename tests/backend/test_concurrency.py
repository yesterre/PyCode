...
def test_index_lock_keeps_health_get_and_other_projects_responsive(
    ...
    first = index_worker.submit(client, ready_url)
    with ThreadPoolExecutor(max_workers=2) as pool:
        ...
        try:
            ...
            for suffix, kwargs in [("ask", {"json": {"question": "entry"}}),
            ...
            competing = index_worker.submit(client, ready_url)
            with pytest.raises(BackendError) as failure:
                index_worker.execute(UUID(competing.json()["task_id"]))
            assert failure.value.code == "project_busy"
            assert index_worker.run(client, other_url).status_code == 200
...
def test_analysis_holds_project_lock_without_blocking_get(
    client, ready_url, llm, monkeypatch, index_worker,
):
    entered, release = Event(), Event()
    ...
    with ThreadPoolExecutor(max_workers=1) as pool:
        ...
        try:
            ...
            assert client.get(ready_url).json()["status"] == "ready"
            assert client.get("/health").status_code == 200
            queued = index_worker.submit(client, ready_url)
            with pytest.raises(BackendError) as failure:
                index_worker.execute(UUID(queued.json()["task_id"]))
            assert failure.value.code == "project_busy"
